...
### Task 0: Create Tax Year Configuration

**Files:** src/tax/year_config.py (new)

**Action:**
Create centralized tax year configuration to avoid hardcoded values:
...
class TaxYearConfig:
    ...
    qbi_phaseout: Decimal = Decimal("50000")
...
print(f'QBI threshold (single): {config.qbi_threshold_single}')
print(f'Standard deduction (MFJ): {config.standard_deduction_mfj}')
"
```

**Done:** TaxYearConfig created with 2024/2025 values
