...
- TaxYearCarryovers dataclass for prior year data

## Tasks

### Task 0: Create Tax Year Carryovers Dataclass

**Files:** src/agents/personal_tax/carryovers.py (new)

**Action:**
Create carryover tracking for prior year amounts:
...
carryovers = TaxYearCarryovers(
    ...
    capital_loss_carryforward_long_term=Decimal('5000'),
    source='prior_year_return',
)
print(f'Has capital loss carryover: {carryovers.has_capital_loss_carryover}')
print(f'Amount: {carryovers.capital_loss_carryforward}')
"
```

**Done:** TaxYearCarryovers dataclass for prior year tracking
