...
class QBIComponent:
    ...
    sstb_reason: str | None = None  # Why it was classified as SSTB (from auto-detect)
...
### Task 4b: Create SSTB Auto-Classification

**Files:** src/agents/personal_tax/sstb.py (new)

**Action:**
Create SSTB (Specified Service Trade or Business) auto-classification:
...
# Test law firm
is_sstb, reason = classify_sstb('541110', 'Offices of lawyers', 'Smith Law Firm')
print(f'Law firm SSTB: {is_sstb}, reason: {reason}')

# Test consulting
is_sstb, reason = classify_sstb('541611', 'Management consulting services', 'ABC Consulting')
print(f'Consulting SSTB: {is_sstb}, reason: {reason}')

# Test regular business
is_sstb, reason = classify_sstb('238220', 'Plumbing, heating, and air-conditioning', 'Johns Plumbing')
print(f'Plumbing SSTB: {is_sstb}, reason: {reason}')
"
```

**Done:** SSTB auto-classification from NAICS codes and keywords
...
**Action:**
Create helpers to build QBI components from income sources (with SSTB auto-detect):
...
def build_qbi_from_schedule_c(
    ...
    return QBIComponent(
        ...
        is_sstb=is_sstb,
        sstb_reason=sstb_reason,  # Track why it was classified
        source="schedule_c",
    )
