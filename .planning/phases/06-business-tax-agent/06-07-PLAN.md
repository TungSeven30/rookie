...
must_haves:
  truths:
    ...
    - "Generated K-1 artifacts (FormK1) are consumable by Personal Tax Agent without validation errors"
  artifacts:
...
<task type="auto">
  <name>Task 3: Verify K-1 handoff is consumable by Personal Tax Agent</name>
  <files>tests/agents/business_tax/test_agent.py</files>
  <action>
Add a test to `tests/agents/business_tax/test_agent.py` (or a dedicated `test_k1_handoff_integration.py` if cleaner) that verifies the Personal Tax Agent can consume generated K-1 artifacts (BTAX-06 verification).

**test_k1_handoff_consumed_by_personal_tax_agent:**
1. Build a sample Form1120SResult with 2 shareholders (reuse fixtures from Task 2)
2. Generate K-1 handoff data using `generate_k1_for_handoff` for each shareholder
3. Serialize each FormK1 using `serialize_k1_artifact`
4. Deserialize each back using `deserialize_k1_artifact`
5. Import PersonalTaxAgent (or its K-1 processing logic)
6. Feed each deserialized FormK1 into the personal tax agent's K-1 intake path
7. Assert: PersonalTaxAgent accepts the FormK1 without validation errors
8. Assert: K-1 box amounts are accessible (ordinary_business_income, interest_income, etc.)
9. Assert: Capital account fields populated from basis_result mapping

This test proves the contract between Business Tax Agent output and Personal Tax Agent input is valid end-to-end.

Use AsyncMock for session if needed. The point is structural validation (FormK1 accepted, fields readable), not full personal tax computation.
  </action>
  <verify>
```bash
cd /Users/tungmbp1423/Projects/rookie && uv run pytest tests/agents/business_tax/test_agent.py -v -k "handoff_consumed" --tb=short
```
  </verify>
  <done>PersonalTaxAgent successfully consumes FormK1 artifacts generated by BusinessTaxAgent. BTAX-06 handoff contract verified.</done>
</task>

<task type="auto">
  <name>Task 4: Run full Phase 6 test suite</name>
  <files></files>
  <action>
